...
const Str parse_datetime_to_minutes(const Str &datetime) {
  ...
  // Copy only digits
  std::copy_if(datetime.begin(), datetime.end(), std::back_inserter(result),
               ::isdigit);
...
void sleep_seconds(const int seconds, const bool verbose) {
  if (verbose) {
    ...
    auto show_countdown = [](const int seconds) {
      ...
      std::cout << "\033[1;33m" << "seconds remaining: " << seconds
                << "\033[0m";
      std::cout.flush();
    };
...
void wait_for_next_mark(const MarkProvider &provider, const bool verbose) {
  ...
  if (delay > max_delay) {
    if (verbose) {
      std::cout << "SKIP the wait for " << delay
                << " seconds, returning now...\n";
    }
