...
std::chrono::system_clock::time_point get_static_time() {
  ...
  const system_clock::time_point fixed_time =
      system_clock::from_time_t(1708000000);
  return fixed_time;
}
...
Results test_wait_for_next_mark() {
  ...
  r.equals(tsnow < datetimelib::timestamp_seconds(),
           "should be one second behind");
